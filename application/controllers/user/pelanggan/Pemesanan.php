<?php 
	...
	class Pemesanan extends CI_Controller
	{
		...
		function index()
		{
			...
			$this->load->view('menu/user/pemesanan_agenbiasa');
		}
		function data()
		{
			if($this->session->userdata('stat') != "login" || $this->session->userdata("status") != "pelanggan biasa"){
			...
			$this->load->view('_partial/headerbiasa');
			$this->load->view('menu/user/data_transaksibiasa',$data);
		}
		function detail_pesanan($no_pesanan)
		{
			...
			$this->load->view('_partial/headerbiasa');
			$this->load->view('menu/user/data_pesanan_userbiasa',$data);
		}
		function tambahkeranjang()
		{
			...
			foreach ($data as $key) {
					# code...
				if ($qty <= 750 && $this->session->userdata("status") != "pelanggan biasa") {
					# code...
					$price = $key->hrg_grosir1;
				}elseif ($qty>750 && $qty<1000 && $this->session->userdata("status") != "pelanggan biasa") {
					# code...
					$price = $key->hrg_grosir2;
				}else if ($qty>=1000 && $this->session->userdata("status") != "pelanggan biasa") {
					# code...
			...
			redirect("user/pelanggan/pemesanan");
		}
		function updatekeranjang()
		{
			...
			foreach ($harga as $key) {
				...
				if ($qty<= 750 && $this->session->userdata("status") != "pelanggan biasa") {
					...
					$code = $this->cart->update($data);
					redirect('user/pelanggan/pemesanan');
				}else if ($qty >0 && $this->session->userdata("status") != "agen"){
					$data = array(
						'rowid'=> $this->input->post('rowid'),
						'qty'=>$qty,
						'price' => $key->harga,
